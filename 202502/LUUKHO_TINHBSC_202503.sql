drop table tam_luuhs_20250303 purge ;
create table tam_luuhs_20250303 as 
       select * from ttkd_bct.bangiao_hoso_tinhbsc a
--        from ttkdhcm_ktnv.bangiao_hoso_tinhbsc a
       where trunc(ngay_bg) between to_date('01/10/2024','dd/mm/yyyy') and to_date('09/03/2025','dd/mm/yyyy')
--        and exists(select 1 from ttkd_bsc.ct_bsc_ptm 
--                    where thang_ptm >= 202411 and nop_du is null and ma_tb = a.ma_tb)
--        and ma_tb = 'nntam0004'
        and ma_tb in('84913080313','84912001196','84915311883','84914785179','84916228768','84814918686','84946719898','84888114168','84846445860')
;

-- MA_GD = '01030886', ma_tb = 'hcm_smartca_00108960'
commit ;
create index tam_luuhs_20250303_matb on tam_luuhs_20250303 (ma_tb) ;

select * from tam_luuhs_20250303 a 
...
where loaitb_id in(1,58,59,61,171) and nop_du is null 
  ...
  and exists(select 1 from ttkd_bct.tam_luuhs_20250303 
...
where thang_ptm >= 202411
  and nop_du is null
  and exists(select 1 from ttkd_bsc.tam_luuhs_20250303 
                where nop_du = 1
                and ma_tb = a.ma_tb)
;
/*
...
update ttkd_bsc.ct_bsc_ptm a set (bs_luukho, a.nop_du, a.ngay_luuhs_ttkd,a.ngay_luuhs_ttvt, ghi_chu)
                          =  ( select distinct '20250303', b.nop_du
                                , b.ngay_bg,  b.ngay_bg, 'BO SUNG 14/01/2025'
                              from tam_luuhs_20250303 b 
                                where b.ma_tb=a.ma_tb and b.ma_gd=a.ma_gd
                                and dichvuvt_id <> 2
                            ) 
where thang_ptm >= 202411 and nop_du is null
and exists(select 1 from tam_luuhs_20250303 b where b.ma_tb=a.ma_tb and dichvuvt_id <> 2)
;
commit ;
*/
update ttkd_bsc.ct_bsc_ptm a set a.bs_luukho = '20250303' , a.nop_du = 1
                                    , a.ngay_luuhs_ttkd = to_date('07/01/2025','dd/mm/yyyy')
                                    , a.ngay_luuhs_ttvt = to_date('07/01/2025','dd/mm/yyyy')
                                    , ghi_chu = 'BO SUNG 14/01/2025'
where exists(select 1 from tam_luuhs_20250303 b 
...
--create table ttkd_bct.nt_tam_luuhs_20250303 as 
-- drop table nt_tam_luuhs_20250303 purge ;
create table nt_tam_luuhs_20250303 as 
...
--                , max(ngay_nop_du) ngay_nop_du, max(ngay_bg)ngay_bg, max(ngay_nop) ngay_nop
        FROM ttkd_bct.tam_luuhs_20250303 a 
...
;
CREATE INDEX nt_tam_luuhs_20250303_HDID ON nt_tam_luuhs_20250303 (hdtb_id ASC) ;
CREATE INDEX nt_tam_luuhs_20250303_matb ON nt_tam_luuhs_20250303 (ma_tb ASC) ;
CREATE INDEX nt_tam_luuhs_20250303_magd ON nt_tam_luuhs_20250303 (ma_gd ASC) ;

/* ----------------------- TEST TRUOC KHI UP -------------------- */
create table tuyenngo.a_ct_bsc_ptm_temp as
...
select * from ttkd_bct.nt_tam_luuhs_20250303 
where loaitb_id in(1,58,59,61,171) 
and econtract_app = 1
;

select * from ttkd_bct.tam_luuhs_20250303 
where loaitb_id in(1,58,59,61,171) 
and econtract_app = 1
;
...
where (nop_du is null or nop_du=0)
 ...
 and exists(select 1 from ttkd_bct.nt_tam_luuhs_20250303 b 
                where b.loaitb_id in(1,58,59,61,171) 
                and b.nop_du=1 and b.ngay_nop_du is not null
                and b.ma_tb=a.ma_tb and b.ma_gd=a.ma_gd and b.hdtb_id=a.hdtb_id
            )
...
where exists(select 1 from ttkd_bct.nt_tam_luuhs_20250303 b 
                where nop_du=1 and b.ngay_nop_du is not null and b.ma_gd like 'HCM-LD%'
                and dichvuvt_id = 2 and to_char(ngay_bg,'yyyymm') >= '202411' and b.ma_tb=a.ma_tb 
            ) 
and thang_ptm = 202411 
