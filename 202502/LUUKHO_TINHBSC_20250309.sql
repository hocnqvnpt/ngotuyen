drop table tam_luuhs_20250309 purge ;
create table tam_luuhs_20250309 as 
...
commit ;
create index tam_luuhs_20250309_matb on tam_luuhs_20250309 (ma_tb) ;

select * from tam_luuhs_20250309 a 
...
where loaitb_id in(1,58,59,61,171) and nop_du is null 
  ...
  and exists(select 1 from ttkd_bct.tam_luuhs_20250309 
...
where thang_ptm >= 202411
  and nop_du is null
  and exists(select 1 from ttkd_bsc.tam_luuhs_20250309 
                where nop_du = 1
                and ma_tb = a.ma_tb)
;
...
--create table ttkd_bct.nt_tam_luuhs_20250309 as 
-- drop table nt_tam_luuhs_20250309 purge ;
create table nt_tam_luuhs_20250309 as 
...
--                , max(ngay_nop_du) ngay_nop_du, max(ngay_bg)ngay_bg, max(ngay_nop) ngay_nop
        FROM ttkd_bct.tam_luuhs_20250309 a 
...
;
CREATE INDEX nt_tam_luuhs_20250309_HDID ON nt_tam_luuhs_20250309 (hdtb_id ASC) ;
CREATE INDEX nt_tam_luuhs_20250309_matb ON nt_tam_luuhs_20250309 (ma_tb ASC) ;
CREATE INDEX nt_tam_luuhs_20250309_magd ON nt_tam_luuhs_20250309 (ma_gd ASC) ;

select * from nt_tam_luuhs_20250309 ;
/* ----------------------- TEST TRUOC KHI UP -------------------- */
...
update ttkd_bsc.ct_bsc_ptm a set (a.bs_luukho, a.nop_du, a.ngay_luuhs_ttkd, a.ngay_luuhs_ttvt)=
                                ( select distinct '20250309', b.nop_du
                           ...
                           --         , loaitb_id, econtract_app
                                  from ttkd_bct.nt_tam_luuhs_20250309 b 
...
select * from ttkd_bct.tam_luuhs_20250309 
where loaitb_id in(1,58,59,61,171) 
and econtract_app = 1
;
...
USING ( select distinct '20250309', ma_tb, ma_gd, hdtb_id, b.nop_du
...
/* ---------- UP THANG N-3 -------------- */
update ttkd_bsc.ct_bsc_ptm a set a.bs_luukho = '20250309' , a.nop_du = 1
                                    , a.ngay_luuhs_ttkd = to_date('09/03/2025','dd/mm/yyyy')
                                    , a.ngay_luuhs_ttvt = to_date('09/03/2025','dd/mm/yyyy')
where exists(select 1 from ttkd_bct.nt_tam_luuhs_20250309 b 
...
where exists(select 1 from ttkd_bct.nt_tam_luuhs_20250309 b 
                where nop_du=1 and b.ngay_nop_du is not null and b.ma_gd like 'HCM-LD%' 
                and dichvuvt_id = 2 and to_char(ngay_bg,'yyyymm') >= '202411' 
                and ma_tb = '84916128079'
                and b.ma_tb=a.ma_tb 
            ) 
