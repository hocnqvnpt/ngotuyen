select * from tuyenngo.thaydoitocdo_202410_bs ;
--drop table tuyenngo.thaydoitocdo_202410_bs_BHHM purge ;
select * from tuyenngo.thaydoitocdo_202410_bs ;

drop table tuyenngo.thaydoitocdo_202410_bs purge ;
drop table tuyenngo.thaydoitocdo_202410_bs_goc purge ;

create table tuyenngo.thaydoitocdo_202410_bs as 
with v_hd as
       ( -- fiber:        
...
--    and ma_gd = 'HCM-TD/00724253'
;

create index thaydoitocdo_202410_bs_hdtbid on thaydoitocdo_202410_bs (hdtb_id) ;
create index thaydoitocdo_202410_bs_thuebaoid on thaydoitocdo_202410_bs (thuebao_id) ;
create index thaydoitocdo_202410_bs_maduan on thaydoitocdo_202410_bs (ma_duan_banhang) ;
create index thaydoitocdo_202410_bs_matb on thaydoitocdo_202410_bs (ma_tb) ;

select * from thaydoitocdo_202410_bs ;

alter table thaydoitocdo_202410_bs 
...
create table temp_tbduan as 
    select thuebao_id, ma_duan from tuyenngo.v_db_duan a
        where exists(select 1 from thaydoitocdo_202410_bs where thuebao_id=a.thuebao_id)
            and ma_duan is not null;
create index temp_tbduan on temp_tbduan (thuebao_id);

alter table thaydoitocdo_202410_bs add ma_duan varchar2(20);
update thaydoitocdo_202410_bs a 
        set ma_duan=(select replace(ma_duan,' ','') from temp_tbduan where thuebao_id=a.thuebao_id);
     
commit ;
select * from thaydoitocdo_202410_bs ; 

alter table thaydoitocdo_202410_bs add dthu_ps_old_1 number ;
commit ;

select ma_tb, count(*)sl from thaydoitocdo_202410_bs group by ma_tb ;
...
update thaydoitocdo_202410_bs a set dthu_ps_old='', dthu_ps_new='', dthu_ps_old_1=''
;
...
and exists (select 1 from thaydoitocdo_202410_bs where ma_tb=a.ma_tb);;


update thaydoitocdo_202410_bs a 
    set dthu_goi_old = (select distinct cuoc_tggoc from a_temp_thftth where ky_cuoc=20240901 and ma_tb=a.ma_tb)
   -- select * from thaydoitocdo_202410_bs a
...
-- Bo sung iptinh
alter table thaydoitocdo_202410_bs add iptinh number;
commit ;
...
select * from a_temp_new a where exists (select 1 from thaydoitocdo_202410_bs where ma_tb=a.ma_tb);
...
rollback ;
-- dthu_duoctinh:
update thaydoitocdo_202410_bs set dthu_duoctinh='';
update thaydoitocdo_202410_bs 
    set dthu_duoctinh=(case when nvl(dthu_goi_new,0)-nvl(dthu_goi_old,0) > 0  
                            then nvl(dthu_goi_new,0)-nvl(dthu_goi_old,0) else 0 end);
commit ;                                                

drop table thaydoitocdo_202410_bs_goc purge ;
create table thaydoitocdo_202410_bs_goc as select * from thaydoitocdo_202410_bs ; -- luu bang truoc khi delete
select * from thaydoitocdo_202410_bs_goc;

delete from thaydoitocdo_202410_bs a
    -- select * from thaydoitocdo_202410_bs a
...
update thaydoitocdo_202410_bs a set lydo_khongtinh_luong='';
update thaydoitocdo_202410_bs a 
    set lydo_khongtinh_luong=lydo_khongtinh_luong||'-Tinh luong theo hs lap moi'
    -- select lydo_khongtinh_luong from thaydoitocdo_202410_bs a
    where exists (select 1 from ttkd_bsc.ct_bsc_ptm where thang_ptm=202410 and loaihd_id=1 and thuebao_id=a.thuebao_id)
       ;

commit ;
--select a.* from thaydoitocdo_202410_bs a where exists (select 1 from tuyenngo.cdbr_ptm_202410_bctuan where loaihd_id=1 and thuebao_id=a.thuebao_id) ;

--select distinct dichvuvt_id from thaydoitocdo_202410_bs ;
...
update thaydoitocdo_202410_bs a set (pbh_ptm_id,manv_ptm,tennv_ptm,ma_to,ten_to,ma_pb,ten_pb,ma_vtcv,loai_ld,nhom_tiepthi)=
