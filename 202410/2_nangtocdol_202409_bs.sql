select * from tuyenngo.thaydoitocdo_202409_bs ;
drop table tuyenngo.thaydoitocdo_202409_bs purge ;
drop table tuyenngo.thaydoitocdo_202409_bs_goc purge ;

create table tuyenngo.thaydoitocdo_202409_bs as
with v_hd as
       ( -- fiber:        
...
create index thaydoitocdo_202409_bs_hdtbid on thaydoitocdo_202409_bs (hdtb_id) ;
create index thaydoitocdo_202409_bs_thuebaoid on thaydoitocdo_202409_bs (thuebao_id) ;
create index thaydoitocdo_202409_bs_maduan on thaydoitocdo_202409_bs (ma_duan_banhang) ;
create index thaydoitocdo_202409_bs_matb on thaydoitocdo_202409_bs (ma_tb) ;

select * from thaydoitocdo_202409_bs ;
select * from tuyenngo.thaydoitocdo_202409_bs ;

alter table thaydoitocdo_202409_bs 
    add ( tien_dvgt_bs number, 
              ...
              nop_du number);
commit ;
create table thaydoitocdo_202409_bs_goc as select * from thaydoitocdo_202409_bs ; -- luu bang truoc khi delete
select * from thaydoitocdo_202409_bs_goc;
select * from tuyenngo.thaydoitocdo_202409_bs ;

delete from thaydoitocdo_202409_bs a
    -- select * from thaydoitocdo_202409_bs a
...
create table temp_tbduan as 
    select thuebao_id, ma_duan from tuyenngo.v_db_duan a
        where exists(select 1 from thaydoitocdo_202409_bs where thuebao_id=a.thuebao_id)
            and ma_duan is not null;
create index temp_tbduan on temp_tbduan (thuebao_id);

alter table thaydoitocdo_202409_bs add ma_duan varchar2(20);
update thaydoitocdo_202409_bs a 
...
/*
update thaydoitocdo_202409_bs a set dthu_ps_old='', dthu_ps_new='';
update thaydoitocdo_202409_bs a 
...
*/
-- dthu_ps_old,dthu_ps_new :
alter table thaydoitocdo_202409_bs add dthu_ps_old_1 number ;
commit ;

select ma_tb, count(*)sl from thaydoitocdo_202409_bs group by ma_tb having count(*)>1 ;
...
update thaydoitocdo_202409_bs a set dthu_ps_old='', dthu_ps_new='', dthu_ps_old_1='' ;
...
-- Bo sung iptinh
alter table thaydoitocdo_202409_bs add iptinh number;
...
-- dthu_duoctinh:
update thaydoitocdo_202409_bs set dthu_duoctinh='';
update thaydoitocdo_202409_bs 
...
update thaydoitocdo_202409_bs a set lydo_khongtinh_luong='';
update thaydoitocdo_202409_bs a 
    set lydo_khongtinh_luong=lydo_khongtinh_luong||'-Tinh luong theo hs lap moi'
    -- select lydo_khongtinh_luong from thaydoitocdo_202409_bs a
    where exists (select 1 from ttkd_bsc.ct_bsc_ptm where thang_ptm=202409 and loaihd_id=1 and thuebao_id=a.thuebao_id)
       or exists(select 1 from ttkd_bct.ptm_cdbr_tam where loaihd_id=1 and  thuebao_id=a.thuebao_id);

commit ;
--select a.* from thaydoitocdo_202409_bs a where exists (select 1 from tuyenngo.cdbr_ptm_202409_bctuan where loaihd_id=1 and thuebao_id=a.thuebao_id) ;

--select distinct dichvuvt_id from thaydoitocdo_202409_bs ;
...
- c?t lydo_khongtinh_luong và c?t lydo_khongtinh_dongia: bs thêm ";Cuoc phat sinh sau khi doi toc do khong cao hon"
- ki?m tra n?u thang_tldg_dt=202409 thì c?p nh?t 4 c?t tr? v? null
...
update thaydoitocdo_202409_bs a set (pbh_ptm_id,manv_ptm,tennv_ptm,ma_to,ten_to,ma_pb,ten_pb,ma_vtcv,loai_ld,nhom_tiepthi)=
...
insert into ttkd_bsc.ct_bsc_ptm 
        ...
        from tuyenngo.thaydoitocdo_202409_bs_bs a
