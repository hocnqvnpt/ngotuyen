select * from ttkdhcm_ktnv.db_act_593 where to_number(to_char(ngay_imp,'yyyymm'))>=202409 ;
...
update ttkd_bsc.ct_bsc_ptm a
    set (tien_dnhm, tien_tt, soseri, ngay_tt)=
            (select round(sotien/1.1,0), round(sotien/1.1,0), so_hd, ngay_thu
                ...
                where to_number(to_char(ngay_imp,'yyyymm'))>=202409 
    ...
    where thang_ptm>=202403 and chuquan_id=264 and loaihd_id=1 and manv_ptm is not null 
          and exists (select 1 from ttkdhcm_ktnv.db_act_593
                      where 
                      to_number(to_char(ngay_imp,'yyyymm'))>=202409 and 
                       sotien>0 and lower(nhomphi) like 'ph_ h_a m_ng%' and username=a.ma_tb)
;    

update ttkd_bsc.ct_bsc_ptm a set (ghi_chu, xacnhan_khkt, thang_xacnhan_khkt)=
        (select max(nd_thu), sum(sotien), 202409 
            from ttkdhcm_ktnv.db_act_593
            where to_number(to_char(ngay_imp,'yyyymm'))>=202409 and (lower(nhomphi) like '%tr_ tr__c%' or lower(nhomphi) like '%c__c ph_t sinh%') 
    ...
    where thang_ptm>=202403 and chuquan_id=264 and xacnhan_khkt is null and thang_tldg_dt is null --and ma_tb='fvn_ht779'
            and exists (select 1 from ttkdhcm_ktnv.db_act_593 
                          where to_number(to_char(ngay_imp,'yyyymm'))>=202409 
...
where thang_ptm>=202403 and manv_ptm is not null
  and chuquan_id=264 and thang_xacnhan_khkt=202409
  and exists (select * from ttkdhcm_ktnv.db_act_593 
              where to_char(ngay_imp,'yyyymm')>='202409' and lower(nhomphi) like '%tr_ tr__c%' and username=a.ma_tb)
...
update ttkd_bsc.ct_bsc_ptm a set thang_luong=5, ghi_chu='Tr? tr??c FTTH 6 THÁNG T?NG 1 THÁNG ',xacnhan_khkt=4884000, thang_xacnhan_khkt=202405 
...
update ttkd_bsc.ct_bsc_ptm a set thang_luong=5, ghi_chu='Tr? tr??c FTTH 7 THÁNG',xacnhan_khkt=2508000, thang_xacnhan_khkt=202404 
