...
where loaitb_id is null and thang = 202408 ;
commit ;


update tuyenngo.ct_ptm_ngoaictr_imp_temp a set COMMAND='insert' where dichvuvt_id in(4,7,8,9) and thang=202408 ;
commit ;

update tuyenngo.ct_ptm_ngoaictr_imp_temp a set dichvu_vt='Vinaphone tra sau' where dichvu_vt = 'VNPts' and thang=202408 ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set COMMAND='roaming' where dichvu_vt = 'VNPts' and thang=202408 ;
commit ;

update tuyenngo.ct_ptm_ngoaictr_imp_temp a set ma_gd=replace(ma_gd, chr(13),'') where thang=202408 ;

rollback ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set a.dichvu_vt = 'Vinaphone tra sau' where a.dichvu_vt = 'VNP tra sau' and thang=202408 ;
commit ;
...
--				select * from tuyenngo.ct_ptm_ngoaictr_imp_temp 
				where thang = 202408 and DICHVU_VT= 'Vinaphone tra sau' 
				;			
commit ;
				...
				WHEN MATCHED THEN
					...
					WHERE a.thang = 202408 and a.loaitb_id = 20
					;
commit ;
rollback ;
...
rollback ;                                               
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TEN_PB = replace(ten_pb, 'P.', '') where instr(ten_pb, 'P.') >0 and thang = 202408;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set dichvu_vt = bo_dau(dichvu_vt) where thang = 202408;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TENKIEU_LD = bo_dau(TENKIEU_LD) where thang = 202408;
commit ;
...
update ct_ptm_ngoaictr_imp a set MA_GD=replace(MA_GD,' ','') 
where thang = 202408;
...
select 70, a.THANG, b.ten_pb, b.ma_pb, b.ten_to, b.ma_to, a.MANV_PTM, a.TENNV_PTM, b.ma_vtcv, b.loai_ld, b.NHOMLD_ID NHOM_TIEPTHI
         ...
         , hdkh.hdkh_id, hdtb.hdtb_id, hdkh.loaihd_id, hdtb.kieuld_id, ma_duan_banhang
            ,(case when a.ma_gd like 'HCM-TT%' and a.dichvuvt_id in(4,7,8,9) then 202408 end) thang_tldg_dt
...
select ma_gd, ma_tb, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202408 and thuebao_id in (10775336)
				and thuebao_id in (select thuebao_id from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202408 and replace(ma_gd, ' ', '') = 'HCM-DV/11080949');

select * from tuyenngo.ct_ptm_ngoaictr_imp_temp a where thang=202408 ;   
select * from ttkd_bsc.ct_bsc_ptm 
where thang_ptm=202408 
and nguon in('Trong co che tinh luong','Ngoai co che tinh luong') ;

delete from ttkd_bsc.ct_bsc_ptm where thang_ptm=202408 and nguon in('Trong co che tinh luong','Ngoai co che tinh luong') ;
commit ;
