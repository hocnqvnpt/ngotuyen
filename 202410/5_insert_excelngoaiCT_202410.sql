...
where ten_kh = 'Công Ty TNHH n??c gi?i khát Coca Cola Vi?t Nam'
;
commit ;
...
where loaitb_id is null and thang = 202410 ;
commit ;
...
where loaitb_id in(149,20) and thang = 202410 ;
commit ;


update tuyenngo.ct_ptm_ngoaictr_imp_temp a set ma_gd=replace(ma_gd, chr(13),'') where thang=202410 ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set MA_GD=replace(MA_GD,' ','') where thang = 202410;
commit ;

rollback ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set a.dichvu_vt = 'Vinaphone tra sau', loaitb_id = 20, dichvuvt_id = 2
where a.dichvu_vt = 'VNP tr? sau' and thang=202410 ;
commit ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set a.dichvu_vt = 'VCC', loaitb_id = 149, dichvuvt_id = 15
where a.dichvu_vt = 'VCC' and thang=202410 ;
commit ;


update tuyenngo.ct_ptm_ngoaictr_imp_temp a set COMMAND='insert' where dichvuvt_id in(4,7,8,9) and thang=202410 ;
commit ;

update tuyenngo.ct_ptm_ngoaictr_imp_temp a set COMMAND='roaming' where dichvu_vt = 'Vinaphone tra sau' and thang=202410 ;
commit ;
...
--				select * from tuyenngo.ct_ptm_ngoaictr_imp_temp 
				where thang = 202410 and DICHVU_VT= 'Vinaphone tra sau' 
				;			
commit ;
...
create index a_temp_loaitbid on a_temp (loaitb_id) ;
				...
				WHEN MATCHED THEN
					...
					WHERE a.thang = 202410 and a.loaitb_id = 20
					;
commit ;
rollback ;
...
where thang=202410 and doituong_kh is null ;
commit ;

rollback ;                                            
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TEN_PB = replace(ten_pb, 'P.', '') where instr(ten_pb, 'P.') >0 and thang = 202410;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set dichvu_vt = bo_dau(dichvu_vt) where thang = 202410;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TENKIEU_LD = bo_dau(TENKIEU_LD) where thang = 202410;
commit ;
update ttkd_bsc.ct_ptm_ngoaictr_imp set TENKIEU_LD = bo_dau(TENKIEU_LD) where thang = 202410;
commit ;

delete from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202410 ;
commit ;
...
select 70, a.THANG, b.ten_pb, b.ma_pb, b.ten_to, b.ma_to, a.MANV_PTM, a.TENNV_PTM, b.ma_vtcv, b.loai_ld, b.NHOMLD_ID NHOM_TIEPTHI
         ...
         ,(case when a.ma_gd like 'HCM-TT%' and a.dichvuvt_id in(4,7,8,9) then 202410 end) thang_tldg_dt
...
update ttkd_bsc.ct_bsc_ptm set thang_tldg_dt = 0, thang_tlkpi = 0, thang_tlkpi_to = 0, thang_tlkpi_phong = 0
where thang_luong = 70 and ma_tb = 'yduoc215'
;
commit ;

select ma_gd, ma_tb, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202410 and thuebao_id in (10775336)
and thuebao_id in (select thuebao_id from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202410 and replace(ma_gd, ' ', '') = 'HCM-DV/11080949');

select * from ttkd_bsc.ct_ptm_ngoaictr_imp a where thang=202410 ;   
select * from ttkd_bsc.ct_bsc_ptm 
where thang_ptm=202410 
and nguon like 'ct_ptm_ngoaictr_imp_%' ;

delete from ttkd_bsc.ct_bsc_ptm where thang_ptm=202410 and nguon in('Trong co che tinh luong','Ngoai co che tinh luong') ;
commit ;
