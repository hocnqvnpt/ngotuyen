...
where loaitb_id is null and thang = 202409 ;
commit ;

update tuyenngo.ct_ptm_ngoaictr_imp_temp a set COMMAND='insert' where dichvuvt_id in(4,7,8,9) and thang=202409 ;
commit ;

update tuyenngo.ct_ptm_ngoaictr_imp_temp a set ma_gd=replace(ma_gd, chr(13),'') where thang=202409 ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set MA_GD=replace(MA_GD,' ','') where thang = 202409;
commit ;

rollback ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set a.dichvu_vt = 'Vinaphone tra sau' where a.dichvu_vt = 'VNP tr? sau' and thang=202409 ;
commit ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set COMMAND='roaming' where dichvu_vt = 'Vinaphone tra sau' and thang=202409 ;
commit ;
...
--				select * from tuyenngo.ct_ptm_ngoaictr_imp_temp 
				where thang = 202409 and DICHVU_VT= 'Vinaphone tra sau' 
				;			
commit ;
...
create index a_temp_loaitbid on a_temp (loaitb_id) ;
				...
				WHEN MATCHED THEN
					...
					WHERE a.thang = 202409 and a.loaitb_id = 20
					;
commit ;
rollback ;
...
where thang=202409 and doituong_kh is null ;
commit ;

rollback ;                                            
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TEN_PB = replace(ten_pb, 'P.', '') where instr(ten_pb, 'P.') >0 and thang = 202409;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set dichvu_vt = bo_dau(dichvu_vt) where thang = 202409;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TENKIEU_LD = bo_dau(TENKIEU_LD) where thang = 202409;
commit ;

delete from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202409 ;
commit ;
...
select 70, a.THANG, b.ten_pb, b.ma_pb, b.ten_to, b.ma_to, a.MANV_PTM, a.TENNV_PTM, b.ma_vtcv, b.loai_ld, b.NHOMLD_ID NHOM_TIEPTHI
         ...
         ,(case when a.ma_gd like 'HCM-TT%' and a.dichvuvt_id in(4,7,8,9) then 202409 end) thang_tldg_dt
...
select ma_gd, ma_tb, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202409 and thuebao_id in (10775336)
and thuebao_id in (select thuebao_id from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202409 and replace(ma_gd, ' ', '') = 'HCM-DV/11080949');

select * from tuyenngo.ct_ptm_ngoaictr_imp_temp a where thang=202409 ;   
select * from ttkd_bsc.ct_bsc_ptm 
where thang_ptm=202409 
and nguon in('Trong co che tinh luong','Ngoai co che tinh luong') ;

delete from ttkd_bsc.ct_bsc_ptm where thang_ptm=202409 and nguon in('Trong co che tinh luong','Ngoai co che tinh luong') ;
commit ;
