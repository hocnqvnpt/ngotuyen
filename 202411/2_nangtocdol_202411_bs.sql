select * from tuyenngo.thaydoitocdo_202411_bs ;
drop table tuyenngo.thaydoitocdo_202411_bs purge ;
drop table tuyenngo.thaydoitocdo_202411_bs_goc purge ;

create table tuyenngo.thaydoitocdo_202411_bs as
with v_hd as
       ( -- fiber:        
...
create index thaydoitocdo_202411_bs_hdtbid on thaydoitocdo_202411_bs (hdtb_id) ;
create index thaydoitocdo_202411_bs_thuebaoid on thaydoitocdo_202411_bs (thuebao_id) ;
create index thaydoitocdo_202411_bs_maduan on thaydoitocdo_202411_bs (ma_duan_banhang) ;
create index thaydoitocdo_202411_bs_matb on thaydoitocdo_202411_bs (ma_tb) ;

select * from thaydoitocdo_202411_bs ;
select * from thaydoitocdo_202411_bs where MA_TD_OLD <> MA_TD_NEW ;

alter table thaydoitocdo_202411_bs 
...
create table temp_tbduan as 
    select thuebao_id, ma_duan from tuyenngo.v_db_duan a
        where exists(select 1 from thaydoitocdo_202411_bs where thuebao_id=a.thuebao_id)
            and ma_duan is not null;
create index temp_tbduan on temp_tbduan (thuebao_id);

alter table thaydoitocdo_202411_bs add ma_duan varchar2(20);
update thaydoitocdo_202411_bs a 
...
/*
update thaydoitocdo_202411_bs a set dthu_ps_old='', dthu_ps_new='';
update thaydoitocdo_202411_bs a 
...
*/
-- dthu_ps_old,dthu_ps_new :
alter table thaydoitocdo_202411_bs add dthu_ps_old_1 number ;
commit ;

select ma_tb, count(*)sl from thaydoitocdo_202411_bs group by ma_tb ;
...
update thaydoitocdo_202411_bs a set dthu_ps_old='', dthu_ps_new='', dthu_ps_old_1=''
;
...
and exists (select 1 from thaydoitocdo_202411_bs where ma_tb=a.ma_tb);


update thaydoitocdo_202411_bs a 
    set dthu_goi_old = (select distinct cuoc_tggoc from a_temp_thftth where ky_cuoc=20241001 and ma_tb=a.ma_tb)
   -- select * from thaydoitocdo_202411_bs a
...
-- Bo sung iptinh
alter table thaydoitocdo_202411_bs add iptinh number;
commit ;
...
select * from a_temp_new a where exists (select 1 from thaydoitocdo_202411_bs where ma_tb=a.ma_tb);
...
-- dthu_duoctinh:
update thaydoitocdo_202411_bs set dthu_duoctinh='';
update thaydoitocdo_202411_bs 
    set dthu_duoctinh=(case when nvl(dthu_goi_new,0)-nvl(dthu_goi_old,0) > 0  
                            then nvl(dthu_goi_new,0)-nvl(dthu_goi_old,0) else 0 end);
commit ;                                                

--drop table thaydoitocdo_202411_bs_goc purge ;
create table thaydoitocdo_202411_bs_goc as select * from thaydoitocdo_202411_bs ; -- luu bang truoc khi delete
select * from thaydoitocdo_202411_bs_goc;

delete from thaydoitocdo_202411_bs a
    -- select * from thaydoitocdo_202411_bs a
...
update thaydoitocdo_202411_bs a set lydo_khongtinh_luong='';
update thaydoitocdo_202411_bs a 
    set lydo_khongtinh_luong=lydo_khongtinh_luong||'-Tinh luong theo hs lap moi'
    -- select lydo_khongtinh_luong from thaydoitocdo_202411_bs a
    where exists (select 1 from ttkd_bsc.ct_bsc_ptm where thang_ptm=202411 and loaihd_id=1 and thuebao_id=a.thuebao_id)
    ;
commit ;
--select a.* from thaydoitocdo_202411_bs a where exists (select 1 from tuyenngo.cdbr_ptm_202411_bctuan where loaihd_id=1 and thuebao_id=a.thuebao_id) ;

--select distinct dichvuvt_id from thaydoitocdo_202411_bs ;
...
update thaydoitocdo_202411_bs a set (pbh_ptm_id,manv_ptm,tennv_ptm,ma_to,ten_to,ma_pb,ten_pb,ma_vtcv,loai_ld,nhom_tiepthi)=
...
insert into ttkd_bsc.ct_bsc_ptm 
        ...
        select  999, 202410 , ma_gd,ma_kh,ma_tb,dich_vu,tenkieu_ld,ten_tb,diachi_ld,so_gt,mst_kh, mst_tt,
        ...
        from tuyenngo.thaydoitocdo_202410_bs a
        where to_char(NGAY_SD,'yyyymm') < '202410' 
          and not exists (select 1 from ttkd_bsc.ct_bsc_ptm where hdtb_id=a.hdtb_id )          
          and loaitb_id!=39 and kieuld_id = 743
        ;
            
commit; 
select * from tuyenngo.thaydoitocdo_202411_bs a where to_char(NGAY_SD,'yyyymm') < '202411' and dichvuvt_id = 4 ;

SELECT * FROM ttkd_bsc.ct_bsc_ptm WHERE NGUON='thaydoitocdo' and thang_luong = 999 ;
/*
