select * from ttkdhcm_ktnv.db_act_593 where to_number(to_char(ngay_imp,'yyyymm'))>=202407;
...
update ttkd_bsc.ct_bsc_ptm a
    set (tien_dnhm, tien_tt, soseri, ngay_tt)=
            (select round(sotien/1.1,0), round(sotien/1.1,0), so_hd, ngay_thu
                ...
                where to_number(to_char(ngay_imp,'yyyymm'))>=202407 
    ...
    where thang_ptm>=202407 and chuquan_id=264 and loaihd_id=1 and manv_ptm is not null AND ma_tb = 'fvn_a1209'
          and exists (select 1 from ttkdhcm_ktnv.db_act_593
                      where 
                      to_number(to_char(ngay_imp,'yyyymm'))>=202407 and 
                       sotien>0 and lower(nhomphi) like 'ph_ h_a m_ng%'  and username = 'fvn_a1209'  --a.ma_tb
                     )
;    

update ttkd_bsc.ct_bsc_ptm a set (ghi_chu, xacnhan_khkt, thang_xacnhan_khkt, trangthai_tt_id)=
        (select max(nd_thu), sum(sotien), 202407, 1 
            from ttkdhcm_ktnv.db_act_593
            where to_number(to_char(ngay_imp,'yyyymm'))>=202407 and (lower(nhomphi) like '%tr_ tr__c%' or lower(nhomphi) like '%c__c ph_t sinh%') 
    ...
    where thang_ptm>=202407 and chuquan_id=264 and xacnhan_khkt is null and thang_tldg_dt is null 
            and exists (select 1 from ttkdhcm_ktnv.db_act_593 
                          where to_number(to_char(ngay_imp,'yyyymm'))>=202407 
...
where thang_ptm>=202407 and manv_ptm is not null
  and chuquan_id=264 and thang_xacnhan_khkt=202407
  and exists (select * from ttkdhcm_ktnv.db_act_593 
              where to_char(ngay_imp,'yyyymm')>='202407' and lower(nhomphi) like '%tr_ tr__c%' and username=a.ma_tb)
...
update ttkd_bsc.ct_bsc_ptm a set thang_luong=5, ghi_chu='Tr? tr??c FTTH 6 TH�NG T?NG 1 TH�NG ',xacnhan_khkt=4884000, thang_xacnhan_khkt=202407 
