select * from tuyenngo.thaydoitocdo_202411 ;
drop table tuyenngo.thaydoitocdo_202411 purge ;
drop table tuyenngo.thaydoitocdo_202411_goc purge ;

create table tuyenngo.thaydoitocdo_202411 as
with v_hd as
       ( -- fiber:        
         SELECT hd.*, hd_adsl.tucthi FROM css.v_hd_thuebao@dataguard hd, css.v_hdtb_adsl@dataguard hd_adsl
           WHERE 
             ...
             AND hd.ngay_kh  BETWEEN TO_DATE('202410', 'yyyyMM') AND ADD_MONTHS(TO_DATE('202411', 'yyyyMM'), 1) - 1/86400
             ...
             AND hd.ngay_kh = (SELECT MAX(ngay_kh)
                               ...
                               WHERE phanvung_id = 28
                                 AND tthd_id IN (5, 6)
                                 AND loaitb_id = 58
                                 AND TO_CHAR(ngay_kh, 'yyyymm') BETWEEN '202410' AND '202411'
         ...
         SELECT hd.*, null as tucthi FROM css.v_hd_thuebao@dataguard hd
           WHERE phanvung_id = 28
             ...
             AND ngay_ht BETWEEN TO_DATE('202411', 'yyyyMM') AND ADD_MONTHS(TO_DATE('202411', 'yyyyMM'), 1) - 1/86400            
             AND ngay_ht = (SELECT MAX(ngay_ht)
                               FROM css.v_hd_thuebao@dataguard hd1
                               WHERE --phanvung_id = 28 AND
                                 TO_CHAR(hd1.ngay_ht, 'yyyymm') = '202411' 
 ...
 ,v_dbtb_new as (select dbtb.thuebao_id, dbtb.thanhtoan_id, dbtb.khachhang_id, dbtb.chuquan_id, dbtb.ten_tb, dbtb.diachi_ld, dbtb.doituong_id
                 ...
                 where dbtb.ky_cuoc = 20241101 and dbtb.phanvung_id = 28 and dbtt.phanvung_id = 28 and dbkh.phanvung_id = 28
 ...
 ,v_dbtb_old as (select thuebao_id, thanhtoan_id, khachhang_id, chuquan_id, ten_tb, diachi_ld,doituong_id, ngay_sd, tocdo_id
                 ...
                 where ky_cuoc = 20241001 and phanvung_id = 28 
...
create index thaydoitocdo_202411_hdtbid on thaydoitocdo_202411 (hdtb_id) ;
create index thaydoitocdo_202411_thuebaoid on thaydoitocdo_202411 (thuebao_id) ;
create index thaydoitocdo_202411_maduan on thaydoitocdo_202411 (ma_duan_banhang) ;
create index thaydoitocdo_202411_matb on thaydoitocdo_202411 (ma_tb) ;

select * from thaydoitocdo_202411 ;
select * from thaydoitocdo_202411 where MA_TD_OLD <> MA_TD_NEW ;

alter table thaydoitocdo_202411 
...
create table temp_tbduan as 
    select thuebao_id, ma_duan from tuyenngo.v_db_duan a
        where exists(select 1 from thaydoitocdo_202411 where thuebao_id=a.thuebao_id)
            and ma_duan is not null;
create index temp_tbduan on temp_tbduan (thuebao_id);

alter table thaydoitocdo_202411 add ma_duan varchar2(20);
update thaydoitocdo_202411 a 
...
/*
update thaydoitocdo_202411 a set dthu_ps_old='', dthu_ps_new='';
update thaydoitocdo_202411 a 
        set dthu_ps_old=(select sum(dthu) from ttkd_bct.cuoc_thuebao_ttkd_202410 where thuebao_id=a.thuebao_id),
            thu_ps_new=(select sum(nogoc) from bcss_hcm.ct_no
                                         where ky_cuoc=20241101 and khoanmuctt_id not in (441,453,520,521,527,3126,3127,3421,3953) -- 453 cuoc GDVP
                                                and thuebao_id=a.thuebao_id);
*/
-- dthu_ps_old,dthu_ps_new :
alter table thaydoitocdo_202411 add dthu_ps_old_1 number ;
commit ;

select ma_tb, count(*)sl from thaydoitocdo_202411 group by ma_tb ;
select ma_tb, count(*)sl from bcss.v_thftth@dataguard
where ky_cuoc=20241101 group by ma_tb having count(*)>1 ;
select * from bcss.v_thftth@dataguard where ky_cuoc=20241101  ;
...
where ky_cuoc=20241001 and khoanmuctt_id not in (441,453,520,521,527,3126,3127,3421,3953)
group by thuebao_id
;
...
select thuebao_id, sum(nogoc)dt from bcss.v_ct_no@dataguard
where ky_cuoc=20241101 and khoanmuctt_id not in (441,453,520,521,527,3126,3127,3421,3953)
group by thuebao_id
;
create index a_temp_new_tbid on a_temp_new (thuebao_id) ;

update thaydoitocdo_202411 a set dthu_ps_old='', dthu_ps_new='', dthu_ps_old_1=''
;

update thaydoitocdo_202411 a 
        set dthu_ps_old=(select dt from a_temp_old where thuebao_id=a.thuebao_id),
            dthu_ps_old_1=(select sum(dthu) from ttkd_bct.cuoc_thuebao_ttkd_202410 where thuebao_id=a.thuebao_id),  
...
where phanvung_id= 28 and ky_cuoc in('20241001','20241101')
and exists (select 1 from thaydoitocdo_202411 where ma_tb=a.ma_tb);


update thaydoitocdo_202411 a 
    set dthu_goi_old = (select distinct cuoc_tggoc from a_temp_thftth where ky_cuoc=20241001 and ma_tb=a.ma_tb)
   -- select * from thaydoitocdo_202411 a
   where loaitb_id = 58 and chuquan_id in (145,264,266) and nvl(dthu_goi_old,0)=0;
 
update thaydoitocdo_202411 a 
    set dthu_goi_new = (select cuoc_tggoc from a_temp_thftth where ky_cuoc=20241101 and ma_tb=a.ma_tb)
    -- select * from thaydoitocdo_202411 a
...
-- Bo sung iptinh
alter table thaydoitocdo_202411 add iptinh number;
commit ;
...
and dbf_row_id =(select max(dbf_row_id) from a_temp_thftth where ky_cuoc = '20241101' and ma_tb = a.ma_tb) 
and exists (select 1 from thaydoitocdo_202411 where ma_tb=a.ma_tb);
create index a_temp_new_matb on a_temp_new (ma_tb) ;

select * from a_temp_new a where exists (select 1 from thaydoitocdo_202411 where ma_tb=a.ma_tb);
...
update thaydoitocdo_202411 a 
   ...
   where loaitb_id in (58) and chuquan_id in (145,264,266) 
        and exists (select 1 from a_temp_thftth where ky_cuoc=20241001 and cuoc_ipgoc=0 and ma_tb=a.ma_tb)
        and exists (select 1 from a_temp_thftth where ky_cuoc=20241101 and cuoc_ipgoc>0 and ma_tb=a.ma_tb);
...
-- dthu_duoctinh:
update thaydoitocdo_202411 set dthu_duoctinh='';
update thaydoitocdo_202411 
    set dthu_duoctinh=(case when nvl(dthu_goi_new,0)-nvl(dthu_goi_old,0) > 0  
                            then nvl(dthu_goi_new,0)-nvl(dthu_goi_old,0) else 0 end);
commit ;                                                

--drop table thaydoitocdo_202411_goc purge ;
create table thaydoitocdo_202411_goc as select * from thaydoitocdo_202411 ; -- luu bang truoc khi delete
select * from thaydoitocdo_202411_goc;

delete from thaydoitocdo_202411 a
    -- select * from thaydoitocdo_202411 a
    where loaitb_id in (58,59) 
                and ( (tucthi='Tuc thi' and to_char(ngay_kh,'yyyymm')='202410') 
                            or (tucthi='Thang sau' and to_char(ngay_kh,'yyyymm')='202411')
                            or exists(select 1 from ttkd_bsc.ct_bsc_ptm where hdtb_id=a.hdtb_id));
 commit ;


-- lydo_khongtinh_luong:
update thaydoitocdo_202411 a set lydo_khongtinh_luong='';
update thaydoitocdo_202411 a 
    set lydo_khongtinh_luong=lydo_khongtinh_luong||'-Tinh luong theo hs lap moi'
    -- select lydo_khongtinh_luong from thaydoitocdo_202411 a
    where exists (select 1 from ttkd_bsc.ct_bsc_ptm where thang_ptm=202411 and loaihd_id=1 and thuebao_id=a.thuebao_id)
    ;
commit ;
--select a.* from thaydoitocdo_202411 a where exists (select 1 from tuyenngo.cdbr_ptm_202411_bctuan where loaihd_id=1 and thuebao_id=a.thuebao_id) ;

--select distinct dichvuvt_id from thaydoitocdo_202411 ;
...
1. thang_ptm=202410 and ma_gd like 'HCM-TD/%' and dthu_ps<=dthu_ps_truoc and (thang_tldg_dt=202411 or thang_tldg_dt is null):
- c?t lydo_khongtinh_luong v� c?t lydo_khongtinh_dongia: bs th�m ";Cuoc phat sinh sau khi doi toc do khong cao hon"
- ki?m tra n?u thang_tldg_dt=202411 th� c?p nh?t 4 c?t tr? v? null
...
update thaydoitocdo_202411 a set (pbh_ptm_id,manv_ptm,tennv_ptm,ma_to,ten_to,ma_pb,ten_pb,ma_vtcv,loai_ld,nhom_tiepthi)=
          (select pb.pbh_id,b.ma_nv,b.ten_nv,b.ma_to,b.ten_to,b.ma_pb,b.ten_pb,b.ma_vtcv,b.loai_ld,nhomld_id
           ...
           where b.thang=202411 and b.donvi='TTKD' and b.ma_pb=pb.ma_pb and pb.active=1 and b.ma_nv=a.ma_tiepthi_new)
    where exists(select 1 from ttkd_bsc.nhanvien b where b.manv_hrm=a.ma_tiepthi_new and b.thang=202411 and b.donvi='TTKD');
 commit ;
...
insert into ttkd_bsc.ct_bsc_ptm 
        ...
        select  202411, 202411 , ma_gd,ma_kh,ma_tb,dich_vu,tenkieu_ld,ten_tb,diachi_ld,so_gt,mst_kh, mst_tt,
        ...
        from tuyenngo.thaydoitocdo_202411 a
        where to_char(NGAY_SD,'yyyymm') < '202411' 
          and not exists (select 1 from ttkd_bsc.ct_bsc_ptm where hdtb_id=a.hdtb_id )          
          and loaitb_id!=39;
            
commit; 
/* --------- UP TRANG THAI THANH TOAN -------------- */
update ttkd_bsc.ct_bsc_ptm a set trangthai_tt_id = (select trangthai from css.v_phieutt_hd@dataguard
                                                        where hdkh_id = a.hdkh_id)
-- select thang_luong, thang_ptm, nguon, trangthai_tt_id from ttkd_bsc.ct_bsc_ptm a
where thang_luong = 999 and thang_ptm >= 202409
and exists(select 1 from css.v_phieutt_hd@dataguard where hdkh_id = a.hdkh_id)
;
commit ;

select * from tuyenngo.thaydoitocdo_202411 a where to_char(NGAY_SD,'yyyymm') < '202411' and dichvuvt_id = 4 ;

SELECT * FROM ttkd_bsc.ct_bsc_ptm WHERE NGUON='thaydoitocdo' and thang_ptm = 202411 ;
/*
update ttkd_bsc.ct_bsc_ptm set lydo_khongtinh_luong='Cuoc phat sinh sau khi doi toc do khong cao hon'
where thang_ptm between 202403 and 202411 
...
where thang_ptm between 202408 and 202411
and thang_tldg_dt is null and dthu_goi > 0
and nguon='thaydoitocdo' 
