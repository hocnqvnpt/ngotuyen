...
CREATE TABLE "TUYENNGO"."KPI_007_202411" 
   (	"THANG" NUMBER, 
...
;
--drop table KPI_007_202411 purge ;
--create table KPI_007_202411 as select * from KPI_007_202411 ;
select * from KPI_007_202411;

select * from ttkd_bsc.bangluong_kpi nv where nv.ma_kpi='HCM_DT_LUYKE_002' and thang = 202411 ;

/* ----------- INSERT NHAN VIEN VAO BANG KPI_007... -------------- */
INSERT INTO KPI_007_202411(THANG, ma_kpi, ma_vtcv, ten_vtcv, ma_to, ma_nv, ten_nv, ma_pb, ten_pb, tinh_bsc, thaydoi_vtcv)
...
;
commit ;
select a.* from KPI_007_202411 a 
...
select * from tuyenngo.KPI_007_202411 ;
...
/* ---------- CHUAN LAI LOAI_KPI ------------- */
update tuyenngo.KPI_007_202411 a set loai_kpi = '' ;
update tuyenngo.KPI_007_202411 a set loai_kpi = ( case when a.ma_vtcv = 'VNP-HNHCM_KHDN_4' then 'KPI_TL'
...
update tuyenngo.KPI_007_202411 a set pgd_manv= '' ;

update tuyenngo.KPI_007_202411 a set pgd_manv=( select distinct ma_nv
...
update tuyenngo.KPI_007_202411 a set soluong_nhanvien= '' ;

update tuyenngo.KPI_007_202411 a set sl_nhanvien= '' ;
update tuyenngo.KPI_007_202411 a set sl_nhanvien=(select count(ma_nv) 
                                                    from tuyenngo.KPI_007_202411 
                                                    where loai_kpi='KPI_NV' 
                                                    and tinh_bsc = 1 and thaydoi_vtcv = 0
                                                    and a.ma_nv=pgd_manv)
where loai_kpi='KPI_PGD_PT' 
;
update tuyenngo.KPI_007_202411 a set slnv_pt= '' ;
update tuyenngo.KPI_007_202411 a set slnv_pt=(select count(ma_nv) 
                                                from tuyenngo.KPI_007_202411 
                                                where loai_kpi='KPI_NV' 
                                                and tinh_bsc = 1 and thaydoi_vtcv = 0
                                                and a.ma_nv=pgd_manv)
...
update tuyenngo.KPI_007_202411 a set tl_manv = '' ;
update tuyenngo.KPI_007_202411 a set tl_manv=(select ma_nv from
                                                        ( select distinct ma_nv, ma_to
                                                            from tuyenngo.KPI_007_202411 b
...
update tuyenngo.KPI_007_202411 a set sl_nhanvien=(select count(ma_nv) from tuyenngo.KPI_007_202411 
                                                  where loai_kpi='KPI_NV'                                                     
...
update tuyenngo.KPI_007_202411 a set slnv_pt=( select count(ma_nv) from tuyenngo.KPI_007_202411 
                                               where loai_kpi='KPI_NV' 
...
commit ;
select * from tuyenngo.KPI_007_202411 ;
select ma_nv, count(*)sl from tuyenngo.KPI_007_202411 group by ma_nv ;
/* ------------- CAP NHAT NHAN VIEN DO TL_PT --------------- */

update KPI_007_202411 set thuchien='', giao='', tlth='', mdht='', nv_dat='', sl_dat='', tlht='', cong_tru='' ;
 commit; 
update KPI_007_202411 a set ( thuchien, giao, tlth, mdht, nv_dat )
...
update tuyenngo.KPI_007_202411 a set sl_dat = 0 where loai_kpi = 'KPI_TL' ; 
update tuyenngo.KPI_007_202411 a set sl_dat= ( select nvl(sl_dat,0)
                                               from
...
update tuyenngo.KPI_007_202411 a set sl_dat = 0 where loai_kpi = 'KPI_PGD_PT' ; 
update tuyenngo.KPI_007_202411 a set sl_dat=(select nvl(sl_dat,0) 
...
;
commit ;
update tuyenngo.KPI_007_202411 a set TLHT = '' where loai_kpi in('KPI_PGD_PT','KPI_TL') ;
update tuyenngo.KPI_007_202411 a set TLHT=( case when ma_vtcv = 'VNP-HNHCM_KHDN_4' and sl_nhanvien > 0
...
commit ;
select * from tuyenngo.KPI_007_202411 ;
update tuyenngo.KPI_007_202411 a set cong_tru ='' where loai_kpi in('KPI_PGD_PT','KPI_TL') ;
commit ;

update tuyenngo.KPI_007_202411 a set cong_tru = ( case when TLHT >= 100 and MDHT >= 100 then +20
...
select THANG, MA_KPI, MA_VTCV, TEN_VTCV, MA_TO, MA_NV, TEN_NV, MA_PB, TEN_PB, LOAI_KPI
                                    , SL_NHANVIEN, SOLUONG_NHANVIEN, SLNV_PT, THUCHIEN, GIAO, MDHT, NV_DAT, TL_MANV, PGD_MANV
                                    , SL_DAT, TLHT, CONG_TRU, tinh_BSC, thaydoi_vtcv
from tuyenngo.KPI_007_202411 a ;
commit ;
...
select a.* from tuyenngo.KPI_007_202411 a ;
...
update ttkd_bsc.bangluong_kpi a 
        set DIEM_CONG = (select nvl(cong_tru,0) from tuyenngo.KPI_007_202411 where cong_tru >= 0 and ma_nv = a.ma_nv)
            , DIEM_TRU = (select nvl(cong_tru,0)*(-1) from tuyenngo.KPI_007_202411 where cong_tru < 0 and ma_nv = a.ma_nv)
--    select * from ttkd_bsc.bangluong_kpi a
    where thang = 202411 and ma_kpi = 'HCM_CL_DHQLY_007' and ma_pb in ('VNP0702300','VNP0702400','VNP0702500')
    --AND MA_NV = 'VNP022082'
;
