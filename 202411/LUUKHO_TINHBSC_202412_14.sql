drop table tam_luuhs_20241214 purge ;
create table tam_luuhs_20241214 as 
...
--            , ngay_nop, ngay_bg, dinhkem, econtract_app -- econtract_app = 1 l� qua App
       ...
       where trunc(ngay_bg) between to_date('01/08/2024','dd/mm/yyyy') and to_date('14/12/2024','dd/mm/yyyy')

;
commit ;
create table tam_luuhs_20241214 as select * from tuyenngo.tam_luuhs_20241214 ;
select * FROM ttkd_bct.tam_luuhs_20241214 a where ma_tb = '84822060551';
select * FROM ttkd_bct.nt_tam_luuhs_20241214 a where ma_tb = '84822060551' ; ma_tb in('hcm_edu_00009891','hcm_edu_lms_00001028') ;

--create table ttkd_bct.nt_tam_luuhs_20241214 as 
-- drop table nt_tam_luuhs_20241214 purge ;
create table nt_tam_luuhs_20241214 as 
...
--                , max(ngay_nop_du) ngay_nop_du, max(ngay_bg)ngay_bg, max(ngay_nop) ngay_nop
        FROM ttkd_bct.tam_luuhs_20241214 a 
...
;
CREATE INDEX nt_tam_luuhs_20241214_HDID ON nt_tam_luuhs_20241214 (hdtb_id ASC) ;
CREATE INDEX nt_tam_luuhs_20241214_matb ON nt_tam_luuhs_20241214 (ma_tb ASC) ;
CREATE INDEX nt_tam_luuhs_20241214_magd ON nt_tam_luuhs_20241214 (ma_gd ASC) ;


select * from ttkd_bct.nt_tam_luuhs_20241214 where to_char(ngay_nop_du,'yyyymm') >= 202408 ;
/* --------------------------------------------- */
...
/* ---------- UP THANG N-3 -------------- */
update ttkd_bsc.ct_bsc_ptm a set a.bs_luukho = '20241214' , a.nop_du = 1
                                    , a.ngay_luuhs_ttkd = to_date('14/12/2024','dd/mm/yyyy')
                                    , a.ngay_luuhs_ttvt = to_date('14/12/2024','dd/mm/yyyy')
where exists(select * from ttkd_bct.nt_tam_luuhs_20241214 b 
