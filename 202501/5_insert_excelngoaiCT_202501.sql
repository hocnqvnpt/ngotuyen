...
where thang = 202501 and trong_ct is null ; 
commit ;
...
where ngay_yc is null and thang = 202501 ;
commit ;
...
rollback;
commit ;
select length(mst) from tuyenngo.ct_ptm_ngoaictr_imp_temp a where thang = 202501 ;
...
where loaitb_id is null and thang = 202501 ;
commit ;
...
where loaitb_id in(149,20) and thang = 202501 ;
commit ;


update tuyenngo.ct_ptm_ngoaictr_imp_temp a set ma_gd=replace(ma_gd, chr(13),'') where thang=202501 ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set MA_GD=replace(MA_GD,' ','') where thang = 202501;
commit ;

rollback ;

update tuyenngo.ct_ptm_ngoaictr_imp_temp a set a.dichvu_vt = bo_dau(dichvu_vt) where thang=202501 ;
commit ;

update tuyenngo.ct_ptm_ngoaictr_imp_temp a set a.dichvu_vt = 'Vinaphone tra sau', loaitb_id = 20, dichvuvt_id = 2
where a.dichvu_vt = 'VNP trả sau' and thang=202501 ;
commit ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set a.dichvu_vt = 'VCC', loaitb_id = 149, dichvuvt_id = 15
where a.dichvu_vt = 'VCC' and thang=202501 ;
commit ;


update tuyenngo.ct_ptm_ngoaictr_imp_temp a set COMMAND='insert' where dichvuvt_id in(4,7,8,9) and thang=202501 ;
commit ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set COMMAND='insert' where loaitb_id in(20) and thang=202501 ;
commit ;

update tuyenngo.ct_ptm_ngoaictr_imp_temp a set COMMAND='roaming' where dichvu_vt = 'Vinaphone tra sau' and thang=202501 ;
commit ;
...
--				select * from tuyenngo.ct_ptm_ngoaictr_imp_temp 
				where thang = 202501 and DICHVU_VT= 'Vinaphone tra sau' 
				;			
commit ;
...
create index a_temp_loaitbid on a_temp (loaitb_id) ;
				...
				WHEN MATCHED THEN
					...
					WHERE a.thang = 202501 and a.loaitb_id = 20
					;
commit ;
rollback ;
...
where thang=202501 and doituong_kh is null ;
commit ;

rollback ;          
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TEN_PB = 'BHKVGD' where ten_pb = 'KVGD' ;
commit ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TEN_PB = replace(ten_pb, 'P.', '') where instr(ten_pb, 'P.') >0 and thang = 202501;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set dichvu_vt = bo_dau(dichvu_vt) where thang = 202501;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TENKIEU_LD = bo_dau(TENKIEU_LD) where thang = 202501;
commit ;
...
select * from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202501 ;
...
select 70, a.THANG, b.ten_pb, b.ma_pb, b.ten_to, b.ma_to, a.MANV_PTM, a.TENNV_PTM, b.ma_vtcv, b.loai_ld, b.NHOMLD_ID NHOM_TIEPTHI
         ...
         ,(case when a.ma_gd like 'HCM-TT%' and a.dichvuvt_id in(4,7,8,9) then 202501 end) thang_tldg_dt
...
where a.thang = 202501 and loaitb_id = 58 
...
where exists(select 1 from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202501 and loaitb_id = 58 and ma_tb = a.ma_tb)
and lydo_khongtinh_dongia NOT like '%kenh CTVXHH' 
;

update ttkd_bsc.ct_bsc_ptm a set thang_tldg_dt = 202501, ghi_chu = 'TRONG/NGOAI CHUONG TRONG - GIAO P.DH DA DUYET'
...
update ttkd_bsc.ct_ptm_ngoaictr_imp a set COMMAND='insert' where loaitb_id in(20) and thang=202501 ;
commit ;
...
select ma_gd, ma_tb, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202501 and thuebao_id in (10775336)
and thuebao_id in (select thuebao_id from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202501 and replace(ma_gd, ' ', '') = 'HCM-DV/11080949');

select * from ttkd_bsc.ct_ptm_ngoaictr_imp a where thang=202501 ;   
select * from ttkd_bsc.ct_bsc_ptm 
where thang_ptm=202501 
and nguon like 'ct_ptm_ngoaictr_imp_%' ;

delete from ttkd_bsc.ct_bsc_ptm where thang_ptm=202501 and nguon in('Trong co che tinh luong','Ngoai co che tinh luong') ;
commit ;
