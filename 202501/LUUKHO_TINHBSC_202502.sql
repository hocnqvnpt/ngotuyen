drop table tam_luuhs_20250203 purge ;
create table tam_luuhs_20250203 as 
       select * from ttkd_bct.bangiao_hoso_tinhbsc a
--        from ttkdhcm_ktnv.bangiao_hoso_tinhbsc a
       where trunc(ngay_bg) between to_date('01/09/2024','dd/mm/yyyy') and to_date('03/02/2025','dd/mm/yyyy')
...
commit ;
create index tam_luuhs_20250203_matb on tam_luuhs_20250203 (ma_tb) ;

select * from tam_luuhs_20250203 a 
...
where loaitb_id in(1,58,59,61,171) and nop_du is null 
  ...
  and exists(select 1 from ttkd_bct.tam_luuhs_20250203 
...
where thang_ptm >= 202410
  and nop_du is null
  and exists(select 1 from ttkd_bsc.tam_luuhs_20250203 
                where nop_du = 1
                and ma_tb = a.ma_tb)
;
/*
...
update ttkd_bsc.ct_bsc_ptm a set (bs_luukho, a.nop_du, a.ngay_luuhs_ttkd,a.ngay_luuhs_ttvt, ghi_chu)
                          =  ( select distinct '20250203', b.nop_du
                                , b.ngay_bg,  b.ngay_bg, 'BO SUNG 14/01/2025'
                              from tam_luuhs_20250203 b 
                                where b.ma_tb=a.ma_tb and b.ma_gd=a.ma_gd
                                and dichvuvt_id <> 2
                            ) 
where thang_ptm >= 202410 and nop_du is null
and exists(select 1 from tam_luuhs_20250203 b where b.ma_tb=a.ma_tb and dichvuvt_id <> 2)
;
commit ;
*/
update ttkd_bsc.ct_bsc_ptm a set a.bs_luukho = '20250203' , a.nop_du = 1
                                    , a.ngay_luuhs_ttkd = to_date('07/01/2025','dd/mm/yyyy')
                                    , a.ngay_luuhs_ttvt = to_date('07/01/2025','dd/mm/yyyy')
                                    , ghi_chu = 'BO SUNG 14/01/2025'
where exists(select 1 from tam_luuhs_20250203 b 
...
--create table ttkd_bct.nt_tam_luuhs_20250203 as 
-- drop table nt_tam_luuhs_20250203 purge ;
create table nt_tam_luuhs_20250203 as 
...
--                , max(ngay_nop_du) ngay_nop_du, max(ngay_bg)ngay_bg, max(ngay_nop) ngay_nop
        FROM ttkd_bct.tam_luuhs_20250203 a 
...
;
CREATE INDEX nt_tam_luuhs_20250203_HDID ON nt_tam_luuhs_20250203 (hdtb_id ASC) ;
CREATE INDEX nt_tam_luuhs_20250203_matb ON nt_tam_luuhs_20250203 (ma_tb ASC) ;
CREATE INDEX nt_tam_luuhs_20250203_magd ON nt_tam_luuhs_20250203 (ma_gd ASC) ;
...
update ttkd_bsc.ct_bsc_ptm a set (a.bs_luukho, a.nop_du, a.ngay_luuhs_ttkd, a.ngay_luuhs_ttvt)=
                           ...
                           --         , loaitb_id, econtract_app
                                  from ttkd_bct.nt_tam_luuhs_20250203 b 
...
select * from ttkd_bct.nt_tam_luuhs_20250203 
where loaitb_id in(1,58,59,61,171) 
and econtract_app = 1
;

select * from ttkd_bct.tam_luuhs_20250203 
where loaitb_id in(1,58,59,61,171) 
and econtract_app = 1
;
...
where (nop_du is null or nop_du=0)
 ...
 and exists(select 1 from ttkd_bct.nt_tam_luuhs_20250203 b 
                where b.loaitb_id in(1,58,59,61,171) 
                and b.nop_du=1 and b.ngay_nop_du is not null
                and b.ma_tb=a.ma_tb and b.ma_gd=a.ma_gd and b.hdtb_id=a.hdtb_id
            )
...
where exists(select 1 from ttkd_bct.nt_tam_luuhs_20250203 b 
                where nop_du=1 and b.ngay_nop_du is not null and b.ma_gd like 'HCM-LD%'
                and dichvuvt_id = 2 and to_char(ngay_bg,'yyyymm') >= '202410' and b.ma_tb=a.ma_tb 
            ) 
and thang_ptm = 202410 
