...
INSERT INTO CT_PTM_NGOAICTR_IMP 
SELECT 202412 THANG, TRONG_CT, NGAY_INS, TEN_PB, MANV_PTM, TENNV_PTM, DICHVU_VT, TENKIEU_LD, MA_GD, MA_TB, MA_KH, SO_HD, TEN_KH
, NGAY_YC, GOI_CUOC_CU, GOI_CUOC_MOI, DTHU_GOI_CU, DTHU_GOI_MOI, DTHU_GOI, CHIPHI_DOITAC, HESO_DICHVU, HESO_TRATRUOC, HESO_KHACHHANG, HESO_HOSO, HESO_HOTRO_NVHOTRO, MANV_HOTRO, DIEN_GIAI, COMMAND, LOAITB_ID, DICHVUVT_ID
, DOITUONG_KH, THUEBAO_ID, GHICHU, MA_DUAN_BANHANG, HESO_QUYDINH_NVPTM, MST, GHINHAN_BSC 
FROM  CT_PTM_NGOAICTR_IMP
WHERE THANG = 2024129999 
;
COMMIT ;

SELECT * FROM CSS_HCM.LOAIHINH_TB ;

update TTKD_BSC.ct_ptm_ngoaictr_imp a set thuebao_id=(select thuebao_id from css_hcm.db_thuebao where ma_tb=a.ma_tb)
where loaitb_id = 358 and thang = 202412 AND GHICHU = '88';
commit ;
...
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set manv_ptm=replace(manv_ptm,' ','') where thang=202412 ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set manv_ptm= 'VNP016794' 
where ten_kh = 'C�ng Ty TNHH n??c gi?i kh�t Coca Cola Vi?t Nam'
;
commit ;
...
rollback;
commit ;
select length(mst) from tuyenngo.ct_ptm_ngoaictr_imp_temp a where thang = 202412 ;
...
where loaitb_id is null and thang = 202412 ;
commit ;

drop table a_tem_vnp purge ;
create table a_tem_vnp as
select ('84'||ma_tb)ma_tb, dichvuvt_id, loaitb_id,thuebao_id from css_hcm.db_thuebao a
where loaitb_id in(20)
...
where loaitb_id in(149,20) and thang = 202412 ;
commit ;


update tuyenngo.ct_ptm_ngoaictr_imp_temp a set ma_gd=replace(ma_gd, chr(13),'') where thang=202412 ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set MA_GD=replace(MA_GD,' ','') where thang = 202412;
commit ;

rollback ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set a.dichvu_vt = 'Vinaphone tra sau', loaitb_id = 20, dichvuvt_id = 2
where a.dichvu_vt = 'VNP trả sau' and thang=202412 ;
commit ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp a set a.dichvu_vt = 'VCC', loaitb_id = 149, dichvuvt_id = 15
where a.dichvu_vt = 'VCC' and thang=202412 ;
commit ;


update tuyenngo.ct_ptm_ngoaictr_imp_temp a set COMMAND='insert' where dichvuvt_id in(4,7,8,9) and thang=202412 ;
commit ;

update tuyenngo.ct_ptm_ngoaictr_imp_temp a set COMMAND='roaming' where dichvu_vt = 'Vinaphone tra sau' and thang=202412 ;
commit ;
...
--				select * from tuyenngo.ct_ptm_ngoaictr_imp_temp 
				where thang = 202412 and DICHVU_VT= 'Vinaphone tra sau' 
				;			
commit ;
...
create index a_temp_loaitbid on a_temp (loaitb_id) ;
				...
				WHEN MATCHED THEN
					...
					WHERE a.thang = 202412 and a.loaitb_id = 20
					;
commit ;
rollback ;
...
where thang=202412 and doituong_kh is null ;
commit ;

rollback ;          
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TEN_PB = 'KHDN1' where ten_pb = 'DN1' ;
commit ;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TEN_PB = replace(ten_pb, 'P.', '') where instr(ten_pb, 'P.') >0 and thang = 202412;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set dichvu_vt = bo_dau(dichvu_vt) where thang = 202412;
update tuyenngo.ct_ptm_ngoaictr_imp_temp set TENKIEU_LD = bo_dau(TENKIEU_LD) where thang = 202412;
commit ;
update ttkd_bsc.ct_ptm_ngoaictr_imp set TENKIEU_LD = bo_dau(TENKIEU_LD) where thang = 202412;
commit ;

update ttkd_bsc.ct_ptm_ngoaictr_imp set dien_giai = dien_giai || '; '|| ghichu 
where thang = 202412
;
commit ;
update ttkd_bsc.ct_ptm_ngoaictr_imp a set (loaitb_id, dichvuvt_id) 
                                = (select loaitb_id, dichvuvt_id from css_hcm.loaihinh_tb where loaihinh_tb = a.dichvu_vt )
-- select * from ttkd_bsc.ct_ptm_ngoaictr_imp a
where thang = 202412 and loaitb_id is null
;
commit ;
update ttkd_bsc.ct_ptm_ngoaictr_imp a set ghinhan_bsc = 1
-- select * from ttkd_bsc.ct_ptm_ngoaictr_imp a
where thang = 202412 and ghinhan_bsc is null
;
commit ;
rollback ;
select * from css_hcm.loaihinh_tb where loaihinh_tb = 'VNPT Invoice' ;
...
select 70, a.THANG, b.ten_pb, b.ma_pb, b.ten_to, b.ma_to, a.MANV_PTM, a.TENNV_PTM, b.ma_vtcv, b.loai_ld, b.NHOMLD_ID NHOM_TIEPTHI
         ...
         ,(case when a.ma_gd like 'HCM-TT%' and a.dichvuvt_id in(4,7,8,9) then 202412 end) thang_tldg_dt
...
select ma_gd, ma_tb, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202412 and thuebao_id in (10775336)
and thuebao_id in (select thuebao_id from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202412 and replace(ma_gd, ' ', '') = 'HCM-DV/11080949');

select * from ttkd_bsc.ct_ptm_ngoaictr_imp a where thang=202412 ;   
select * from ttkd_bsc.ct_bsc_ptm 
where thang_ptm=202412 
and nguon like 'ct_ptm_ngoaictr_imp_%' ;

delete from ttkd_bsc.ct_bsc_ptm where thang_ptm=202412 and nguon in('Trong co che tinh luong','Ngoai co che tinh luong') ;
commit ;
