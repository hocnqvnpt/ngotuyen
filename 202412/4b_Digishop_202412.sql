  ...
  CREATE TABLE "TTKD_BSC"."DIGISHOP_202412" 
   (	"THANG" CHAR(6 BYTE), 
	"NGAY_TAO_DON" DATE, 
...
from ttkd_bsc.ct_bsc_ptm where thang_ptm in(202412) ;

select * from ttkd_bsc.ct_bsc_ptm where thang_ptm=202412 and dich_vu='MyTV MOBILE' ;
-- delete from ttkd_bsc.ct_bsc_ptm where thang_ptm=202412 and dich_vu='MyTV MOBILE' ;
commit ;
-- Digishop (MyTV Mobile)
--delete from ct_bsc_ptm where thang_ptm=202412 and dich_vu='MyTV MOBILE';
...
from 
(
    select 202412 thang_luong, 202412 thang_ptm, danhmuc dich_vu
            ...
            , 202412 thang_tldg_dt, 202412 thang_tlkpi, 202412 thang_tlkpi_to, 202412 thang_tlkpi_phong  
    from ttkd_bsc.digishop a, ttkd_bsc.nhanvien b    
    where a.thang=202412 
    and a.ma_gioithieu=b.ma_nv 
    and b.thang=202412 and b.donvi='TTKD'
...
select DISTINCT DANHMUC from ttkd_bsc.digishop_202412 ;
...
--thang_ptm >= 202401 and ma_tb in('84834753888','84822999590','84828666439','84828500131','84843313338','84852345448','84947681782','84948227801','84949019916')
 ...
 where thang_luong=9 
 and exists(select 1 from ttkd_bsc.digishop where a.ma_tb=ma_dhsx and thang=202412 ) ;
 commit ;
 rollback ;
