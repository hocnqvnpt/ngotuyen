...
update KPI_BRVNP_001_202412 a set sl_fiber=0, sl_mytv=0, sl_sl_wfm=0, sl_vnpts=0, tong_ptm = 0 ;
update KPI_BRVNP_001_202412 a set (sl_fiber, sl_mytv, sl_wfm, sl_vnpts,tong_ptm)
                            = ( select nvl(sl_fiber,0), nvl(sl_mytv,0), nvl(sl_wfm,0), nvl(sl_vnpts,0)
                                    , (nvl(sl_fiber,0) + nvl(sl_mytv,0) + round(nvl(sl_wfm,0)/2,0) + nvl(sl_vnpts,0))
...
update KPI_BRVNP_001_202412 a set (sl_fiber, sl_mytv, sl_wfm, sl_vnpts, tong_ptm)
                                 = ( select sl_fiber, sl_mytv, sl_wfm, sl_vnpts
                                         , (nvl(sl_fiber,0) + nvl(sl_mytv,0) + round(nvl(sl_wfm,0)/2,0) + nvl(sl_vnpts,0))
                                     from BHOL_temp_to b
...
update KPI_BRVNP_001_202412 a set (sl_fiber, sl_mytv, sl_vnpts, tong_ptm)
...
update ttkd_bsc.bangluong_kpi a set mucdo_hoanthanh = (select mucdo_hoanthanh from bangluong_kpi_bhol    
                                                where thang = 202412 and nvl(mucdo_hoanthanh,0) > 0 and ma_nv = a.ma_nv and ma_kpi = a.ma_kpi)
...
update ttkd_bsc.bangluong_kpi a set thuchien = (select thuchien from bangluong_kpi_bhol    
                                                where thang = 202412 and nvl(thuchien,0) > 0 and ma_nv = a.ma_nv and ma_kpi = a.ma_kpi)
where thang = 202412 and ma_pb = 'VNP0703000' ;
commit ;
