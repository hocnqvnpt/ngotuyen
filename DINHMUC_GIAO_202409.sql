...
CREATE TABLE "TTKD_BSC"."DINHMUC_GIAO_DTHU_PTM_202409" 
   (	"THANG" NUMBER, 
...
;   
select * from ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 ;

insert into ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 (THANG, MA_NV, TEN_NV, MA_TO, TEN_TO, TEN_VTCV, MA_VTCV, MA_PB, TEN_PB, tinh_bsc, THAYDOI_VTCV)
...
update ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a set 
...
=(
    ...
    from (select * from TTKDHCM_KTNV.ID430_DANGKY_CHOTTHANG where thang = 202409) a
    order by 1,2,3
    ) 
    where thang = a.thang and ma_nv = a.ma_nv)
where a.thang = 202409 ;
commit ;
update ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a set (dinhmuc_1, dinhmuc_2, dinhmuc_3)
                                                = (select dinhmuc_1, dinhmuc_2, dinhmuc_3
                                                    from ttkd_bsc.BLDG_DANHMUC_VTCV_P1 b
                                                    where b.thang = 202409 and b.ma_vtcv = a.ma_vtcv)
where a.thang = 202409
;
commit ;
select distinct a.ma_vtcv, a.ten_vtcv from ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a 
where not exists(select 1 from ttkd_bsc.BLDG_DANHMUC_VTCV_P1 b
                    where b.thang = 202409 and b.ma_vtcv = a.ma_vtcv)
;
/* ---------- UP LAI TINH_BSC, THAYDOI_VTCV --------------- */

update ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a set (tinh_bsc_new, thaydoi_vtcv_new)
                                            = (select tinh_bsc, thaydoi_vtcv 
                                                from ttkd_bsc.nhanvien 
                                                where thang = 202409 and donvi = 'TTKD' and ma_nv = a.ma_nv)
where a.thang = 202409 
;
commit ;

update ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a set loai_kpi
...
;
commit ;
SELECT DISTINCT MA_VTCV, TEN_VTCV, LOAI_KPI FROM ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 ;

SELECT DISTINCT MA_PB, TEN_PB FROM ttkd_bsc.dinhmuc_giao_dthu_ptm_202409  ;
DELETE FROM ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 
WHERE MA_PB IN('VNP0700100','VNP0700200','VNP0700600','VNP0700700','VNP0700900','VNP0700400','VNP0700800','VNP0700500','VNP0700300') ;
COMMIT ;

SELECT DISTINCT MA_VTCV, TEN_VTCV FROM ttkd_bsc.dinhmuc_giao_dthu_ptm_202409
WHERE thang = 202409 and  LOAI_KPI = 'KPI_NV' ;

update ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a set tt_manv=(select ma_nv from
                                                        ( select distinct ma_nv, ma_to, ten_to, thang
                                                            from ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 
                                                            where loai_kpi in('KPI_TT','KPI_CHT','KPI_CHT_GDV','KPI_TL') and thang = 202409                                                        
                                                        ) b
                                                       where b.ma_to=a.ma_to and thang = 202409) 
-- select * from ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a
...
where thang = 202409 and loai_kpi in('KPI_TT','KPI_CHT') --and tinh_bsc = 1 and thaydoi_vtcv = 0
...
where thang = 202409 and loai_kpi = 'KPI_TT' and upper(bo_dau(ten_to)) = 'TO BAN HANG ONLINE'
--and tinh_bsc = 1 and thaydoi_vtcv = 0
--and loai_kpi not in('KPI_NV','KPI_CHT_GDV') 
;
commit ;
...
select * from ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a where KHDK = CANHAN_GIAO ;
update ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a set KHDK = CANHAN_GIAO
where thang = 202409 and CANHAN_GIAO > 0 ;
commit ;
update ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a set KHDK = '' ;
update ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a set KHDK = CANHAN_GIAO
where thang = 202409 and CANHAN_GIAO > 0 and KHDK = 0 ;
commit ;

/* ------------ UP LAI KHDK = 0 KHI TONG_DTGIAO is null and dinhmuc_1 is not null ----------- */
update ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 a set KHDK = 0
-- select * from ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 
where thang = 202409 and TONG_DTGIAO is null and KHDK is null ; -- and dinhmuc_1 is not null 
commit ;
...
select * from ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 where thang = 202409 ;
commit ;
...
update ttkd_bsc.dinhmuc_giao_dthu_ptm a set (loai_kpi, tt_manv) 
                                        = (select loai_kpi, tt_manv 
                                            from ttkd_bsc.dinhmuc_giao_dthu_ptm_202409
                                            where ma_nv = a.ma_nv and thang = 202409)
where thang = 202409 
;
commit ;
select distinct ma_pb, ten_pb from ttkd_bsc.dinhmuc_giao_dthu_ptm_202409 
